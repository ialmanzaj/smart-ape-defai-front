  ...
  const fetchWallet = async (address) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/wallet?chain=${globalDataCache.selectedChain ? globalDataCache.selectedChain : "eth"}`,
        {
          method: "POST",
          headers: {
          ...
          <div className="container text-center" style={{ padding: "100px 0" }}>
            ...
            <div id="wallet-container">
              {loading ? (
                ...
                <>
                  <p>
                    Explore token balances, NFT holdings, activity and insights
                    for any EVM wallet 🔥
                  </p>
                  <WalletForm
