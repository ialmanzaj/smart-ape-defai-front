...
interface SpamResults {
  ...
  spamNFTs: number;
  verifiedNFTs: number;
  tokenSpamChart: { labels: string[]; data: number[] };
  nftSpamChart: { labels: string[]; data: number[] };
  hasMorePages: boolean;
}

const SpamChecker: React.FC = () => {
  const { globalDataCache, setGlobalDataCache } = useData();
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [spamResults, setSpamResults] = useState<SpamResults | null>(null); // Using state to track spam results
  ...
  const fetchWallet = async (address: string) => {
    try {
      ...
      if (response.ok) {
        const data: SpamResults = await response.json();
        setSpamResults(data);
      } else {
